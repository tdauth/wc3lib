...
BOOST_AUTO_TEST_CASE(EntryMpq)
{
	...
	BOOST_CHECK(!source.sources().begin()->isDirectory(0));
...
BOOST_AUTO_TEST_CASE(ExistsMpq)
{
	...
	BOOST_REQUIRE(source.sources().size() == 1);
	BOOST_CHECK(source.exists(KUrl("ReplaceableTextures\\TeamColor00.blp"), 0));
}
...
BOOST_AUTO_TEST_CASE(DownloadMpq)
{
	...
	BOOST_REQUIRE(source.sources().size() == 1);
	QString file;
	BOOST_CHECK(source.download(KUrl("ReplaceableTextures\\TeamColor00.blp"), file, 0));
...
BOOST_AUTO_TEST_CASE(ExistsDir)
{
	...
	BOOST_REQUIRE(source.sources().size() == 1);
	BOOST_CHECK(source.exists(KUrl("test.mpq"), 0));
}
...
BOOST_AUTO_TEST_CASE(DownloadDir)
{
	...
	BOOST_REQUIRE(source.sources().size() == 1);
	QString file;
	BOOST_CHECK(source.download(KUrl("test.mpq"), file, 0));
	BOOST_CHECK(QFileInfo(file).exists());
}
